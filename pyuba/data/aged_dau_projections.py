...
from pyuba.data import cohort_projections
...
def get_DNU(forward_DAU):
    ...
    DNU_df.loc[len(DNU_df)] = DNU_list + (
        [0] * (len(list(DNU_df.columns)) - len(forward_DAU))
    )
...
def project_aged_DAU(profile, periods, cohorts, ages, start_date=1):
    ...
    aged_DAU = pd.DataFrame(columns=["age"] + dates, dtype="float").set_index("age")
    # remove any ages that are > the number of periods being projected out
    ages = [age for age in ages if age <= periods]
    for i, age in enumerate(ages):
        this_age = pd.DataFrame(columns=["age"] + dates, dtype="float").set_index("age")
    ...
    for i, cohort in enumerate(cohorts):
        ...
        for j, age in enumerate(ages):
            ...
            cohort_age = pd.DataFrame(columns=["age"] + dates, dtype="float").set_index(
                "age"
            )
