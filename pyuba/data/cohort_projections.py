...
def project_cohort(cohort, profile, periods):
    ...
    this_cohort = [
        ...
        if i <= max(profile["retention_projection"][0])
        else 0
...
def DAU_total(forward_DAU):
    if not isinstance(forward_DAU, pd.DataFrame) or len(forward_DAU) < 2:
        raise Exception(
            "Forward DAU Projection is malformed. Must be a dataframe with at least 2 rows."
        )
...
def combine_DAU(DAU_totals, labels=None):
    ...
    if labels is not None and (len(DAU_totals) != len(labels)):
        raise Exception(
            "Number of labels doesnt match number of DAU projections provided."
        )
    ...
    for DAU_total in DAU_totals[1:]:
        ...
        common = list(
            set(combined_DAU.columns.tolist()) & set(DAU_total.columns.tolist())
        )
        combined_DAU = pd.merge(combined_DAU, DAU_total, on=common, how="outer").fillna(
            0
        )
        i += 1
    ...
    columns = sorted(
        [
            int(c)
            for c in combined_DAU.columns
            if c not in ["DAU", "profile", "cohort_date", "age", "Value"]
        ]
    )
...
def project_targeted_DAU(
    ...
    model = build_DAU_trajectory(
        start_DAU, DAU_target, (DAU_target_timeline + 1 - len(cohorts))
    )
    ...
    DAU_values = [
        int(model[0] * i + model[1])
        for i in range(1, (DAU_target_timeline + 2 - len(cohorts)))
    ][1:]

    for DAU_target in DAU_values:

        if len(forward_DAU) > len(forward_DAU.columns.tolist()):
            forward_DAU[str(int(forward_DAU.columns.tolist()[-1]) + 1)] = [0] * (
                len(forward_DAU)
            )
        ...
        forward_DAU = build_forward_DAU(
            profile, forward_DAU, DAU_needed, periods, start_date
        )
...
def project_cohorted_DAU(
    profile, periods, cohorts, DAU_target=None, DAU_target_timeline=None, start_date=1
):
    ...
    if (
        len(cohorts) < 1
        or not all(isinstance(x, int) for x in cohorts)
        or not all(x >= 1 for x in cohorts)
    ):
        raise Exception(
            "Must provide at least one cohort value, and all cohort values must be greater than 0"
        )

    if (
        DAU_target is not None
        and DAU_target_timeline is not None
        and DAU_target_timeline > periods
    ):
        raise Exception(
            "DAU target timeline is longer than the number of periods being projected"
        )
    ...
    forward_DAU = pd.DataFrame(columns=["cohort_date"] + dates, dtype=np.int)
    # build the initial forward DAU from the cohorts
    for cohort in cohorts:
        forward_DAU = build_forward_DAU(
            profile, forward_DAU, cohort, periods, start_date
        )
