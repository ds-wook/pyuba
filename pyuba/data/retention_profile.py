...
def generate_retention_profile(profile, profile_max) -> Tuple[np.ndarray, np.ndarray]:
...
def project_retention(profile, profile_max=None, start=None, stop=None) -> np.ndarray:
    ...
    if profile_max is None and start is None and stop is None:
        ...
        raise Exception(
            "Incorrect parameter set sent. Must include either profile_max or start and end points."
        )
    ...
    if this_process != "interpolate":
        ...
        if this_process in curve_functions.processes:
            retention_projection = getattr(curve_functions, this_process + "_func")(
        ...
        if profile_max is None:
            ...
            retention_projection = [
                profile["interpolation_f"](z)
                for z in range(min(profile["x"]), max(profile["x"]) + 1)
            ]
        else:
            #  a profile_max was provided so we need to extrapolate the interpolation out
            #  use the interpolation_s function
            retention_projection = [
                profile["interpolation_s"](z)
                for z in range(min(profile["x"]), max(x2) + 1)
            ]
    retention_projection = [z if z > 0 else 0 for z in retention_projection]

    # replace all negative y values with 0
    retention_projection[1] = np.where(
        retention_projection[1] < 0, 0, retention_projection[1]
    )

    return retention_projection


def generate_curve_coefficients(profile, process_value) -> float:
    if process_value in curve_functions.processes:
        ...
        try:
            ...
            print(
                "Notice: Unable to process retention curve with "
                + process_value
                + " function"
            )
            return None
...
def process_retention_profile_projection(profile) -> Dict[str, float]:
    curve_fit_values = {}
    process_list = curve_functions.processes.copy() + ["interpolate"]

    for process_value in process_list:
        curve_fit_values[process_value] = generate_curve_coefficients(
            profile, process_value
        )
...
def get_retention_projection_best_fit(profile, profile_max=None):
    ...
    for process_value in curve_functions.processes:
        if process_value in profile["params"]:
            ...
            equations[process_value] = getattr(curve_functions, this_func)(
                x2, *this_params
            )
            ...
            for i, x_projection in enumerate(x2):
                ...
                this_ss = sum(
                    [abs(projected_value - this_value) ** 2 for this_value in y_values]
                )
