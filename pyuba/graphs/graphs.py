...
def plot_retention(profile, show_average_values=True):
    ...
    plt.plot(
        retention_projection[0],
        retention_projection[1],
        "m--",
        label=profile_label,
        linewidth=4,
    )
    ...
    if show_values:
        for axis in axes:
            for bar in axis:
                ...
                if h != 0:
                    plt.text(
                        text_loc_x, text_loc_y, h, ha="center", va="center", fontsize=22
                    )
            ...
            if len(totals) == len(category_labels):
                ...
                for index, total in enumerate(totals):
                    ...
                    if count % totals_skip == 0 or count == 0:
                        plt.text(
                            index,
                            total + (totals_height / 100 * sum(totals) / len(totals)),
...
def plot_forward_DAU_stacked(
    ...
    totals = [
        forward_DAU[column].sum()
        for column in forward_DAU.loc[:, forward_DAU.columns != "cohort_date"]
    ]
    ...
    stacked_bar(
        ...
        grid=True,
        reverse=False,
        show_totals_values=show_totals_values,
        totals=totals,
    )
